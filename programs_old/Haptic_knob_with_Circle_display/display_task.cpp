...
struct PB_SmartKnobState {
  int rot;
  int num_of_detent;
};

int PIN_LCD_BACKLIGHT = 2;
...
/*
        ...
        if (state.num_of_detent > 1) {
          int32_t height = state.rot * TFT_HEIGHT / (state.num_of_detent - 1);
          spr_.fillRect(0, TFT_HEIGHT - height, TFT_WIDTH, height, FILL_COLOR);
        }

        spr_.setFreeFont(&Roboto_Light_60);
        int VALUE_OFFSET = 10;
        spr_.drawString(String() + state.rot, TFT_WIDTH / 2, TFT_HEIGHT / 2 - VALUE_OFFSET, 1);
