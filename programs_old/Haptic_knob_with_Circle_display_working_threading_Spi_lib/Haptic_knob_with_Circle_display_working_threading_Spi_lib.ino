...
void motorTaskCode(void *pvParameters) {
  ...
  numDisplay.setZones(75, 100, 50, 75, 25, 50, 0, 25); // Example here red starts at 75% and ends at 100% of full scale
  // Meter is 239 pixels wide and 126 pixels high
  numDisplay.analogMeter(0, 0, 2.0, "mA", "0", "0.5", "1.0", "1.5", "2.0");    // Draw analogue meter at 0, 0
  ...
  */
  ...
  for(;;) {
    ...
    if (xQueueReceive(structQueue, &currentState, portMAX_DELAY) == pdPASS) {
     ...
     // vTaskDelay(100);
      //delay(100);
      float value = map(currentState.rot, 0, PI * 100, 0, 200);
      value = value / 100;
      numDisplay.updateNeedle(value, 0);
      /*
